...
export default function HomeScreen() {
  ...
  const FactCard = ({ fact }: { fact: FactType }) => (
    <TouchableOpacity activeOpacity={0.9} onPress={() => setSelectedFact(fact)} style={styles.cardWrapper}>
      <ThemedView style={styles.smallCard}>
        <Image source={fact.image} style={styles.coffeeImage} contentFit="contain" />
        <ThemedText style={[styles.smallCardText, { color: text }]}>{fact.shortFact}</ThemedText>
      </ThemedView>
    </TouchableOpacity>
  );
    ...
    <ThemedView style={styles.root}>
      ...
      <ThemedView style={[styles.customHeader, { backgroundColor: Colors.light.background }]}>
        ...
        <View style={[styles.headerSide, styles.headerSideRight]}>
          <TouchableOpacity onPress={() => router.push("/(auth)/profile")} activeOpacity={0.7}>
            <Image source={avatarUrl ? { uri: avatarUrl } : defaultAvatarSource} style={styles.avatarImage} contentFit="cover" />
          </TouchableOpacity>
        </View>
      </ThemedView>
      ...
      <ThemedView style={styles.body}>
        ...
        <ThemedView style={[styles.darkCard, { backgroundColor: Colors.light.surface }]}>
          ...
          <TouchableOpacity style={styles.devOtgToggle} onPress={() => setIsOtgConnected((prev) => !prev)}>
      ...
      {/* ── Fact detail modal ── */}
      <Modal visible={!!selectedFact} animationType="slide" transparent onRequestClose={() => setSelectedFact(null)}>
                ...
                <FlatList
                  ...
                  renderItem={({ item }) => (
                    <View>
                      ...
                      <ThemedText style={[styles.modalSuggestion, { color: coffee }]}>suggestion: {item.suggestion}</ThemedText>
                      <ThemedText style={[styles.modalReferencesTitle, { color: coffee }]}>references</ThemedText>
                      {item.references.map((ref, index) => (
                        <ThemedText key={index} style={[styles.modalReference, { color: text }]}>• {ref}</ThemedText>
                      ))}
                    </View>
                  )}
                />
                <Pressable style={[styles.closeButton, { backgroundColor: coffee }]} onPress={() => setSelectedFact(null)}>
      ...
      <Modal
        ...
        transparent
        onRequestClose={() => {
          if (analyzeStatus !== "analyzing") { setAnalyzeModalVisible(false); setAnalyzeStatus("idle"); }
        }}
      >
        <View style={styles.modalOverlay}>
          <View style={styles.analyzeModalContent}>
            {analyzeStatus === "no-device" && (
              <>
                <ThemedText style={[styles.analyzeModalTitle, { color: coffee }]}>insert device</ThemedText>
                <ThemedText style={[styles.analyzeModalText, { color: text }]}>
                  No OTG-connected device was detected. Please insert the device first before analyzing.
                </ThemedText>
                <Pressable style={[styles.actionButton, { backgroundColor: coffee }]} onPress={() => { setAnalyzeModalVisible(false); setAnalyzeStatus("idle"); }}>
            ...
            {analyzeStatus === "analyzing" && (
              <>
                ...
                <ThemedText style={[styles.analyzeModalTitle, { color: coffee, marginTop: 16 }]}>analyzing...</ThemedText>
                <ThemedText style={[styles.analyzeModalText, { color: text }]}>
            ...
            {analyzeStatus === "done" && (
              <>
                <ThemedText style={[styles.analyzeModalTitle, { color: coffee }]}>analysis complete</ThemedText>
                <ThemedText style={[styles.analyzeModalText, { color: text }]}>
                  Your result is ready. Tap the button below to view your latest analysis.
                </ThemedText>
                <Pressable style={[styles.actionButton, { backgroundColor: coffee }]} onPress={() => { setAnalyzeModalVisible(false); setAnalyzeStatus("idle"); router.push("/(tabs)/results"); }}>
...
const styles = StyleSheet.create({
  ...
  darkCard: {
    borderRadius: 20,
    paddingVertical: 0,   // clean — no extra paddingTop stacked on children
  ...
  greetingText: {
    ...
    fontSize: 19,
    fontWeight: "600",
    marginTop: 6,          // space from illustration
    marginBottom: 45,      // space to button
    textAlign: "center",
  },
  ...
  analyzeButton: {
    ...
    borderRadius: 30,
    // no extra marginTop — greetingText marginBottom handles spacing
  },
  analyzeButtonText: {
    fontSize: 15,
    fontWeight: "700",
  ...
  devOtgToggleText: { fontSize: 11, opacity: 0.75, textTransform: "lowercase" },
  ...
  smallCardText: { fontSize: 12, textAlign: "center", fontWeight: "500", lineHeight: 17, paddingHorizontal: 4 },
  ...
  modalText:            { fontSize: 15, lineHeight: 22, marginBottom: 14, textAlign: "justify" },
  modalSuggestion:      { fontSize: 14, lineHeight: 20, fontWeight: "600", marginBottom: 16 },
  modalReferencesTitle: { fontSize: 15, fontWeight: "700", marginBottom: 8, textTransform: "lowercase" },
  modalReference:       { fontSize: 13, lineHeight: 19, marginBottom: 6 },

  closeButton:     { marginTop: 16, paddingVertical: 12, borderRadius: 30, alignItems: "center" },
  closeButtonText: { color: "#F5F5F5", fontSize: 15, fontWeight: "600", textTransform: "lowercase" },
  ...
  analyzeModalTitle: { fontSize: 20, fontWeight: "700", textAlign: "center", textTransform: "lowercase", marginBottom: 10 },
  analyzeModalText:  { fontSize: 15, lineHeight: 22, textAlign: "center", marginBottom: 18 },
  ...
  actionButtonText: { color: "#F5F5F5", fontSize: 15, fontWeight: "600", textTransform: "lowercase" },
});